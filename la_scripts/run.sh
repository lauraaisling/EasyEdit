...
#SBATCH --job-name=DINM_all
#SBATCH --out="la_scripts/log_safeedit_DINM_SafeEdit_test_all.txt" 
...
#Note: To run DINM
python examples/run_safety_editing.py --editing_method=DINM --edited_model=llama-7b --hparams_dir=./hparams/DINM/llama-7b --data_path=./data/SafeEdit_test_ALL.json --safety_classifier_dir=zjunlp/SafeEdit-Safety-Classifier --metrics_save_dir=./safety_results/DINM/all --ckpt_save_dir=safety_results/models/llama-7b-DIMN-all
...
#Note: To run MEND
# python la_scripts/run_mend_zsre_llama-7b.py
...
##to run script with slurm: sbatch -p qgpu-debug la_scripts/run.sh
